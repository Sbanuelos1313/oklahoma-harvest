...
def place_order(req: PlaceOrderRequest, user=Depends(get_current_shopper)):
    ...
    for item in req.items:
        ...
        if p_producer_id != req.producer_id: cur.close(); conn.close(); raise HTTPException(400, f"Product '{p_name}' is from a different producer. All items must be from the same producer.")
    ...
    return {"order_id": order_id, "total": total, "status": "pending", "respond_by": respond_by.isoformat(), "message": "Order placed — producer has 12 hours to confirm"}
...
def update_order_status(order_id: int, req: UpdateOrderStatusRequest, user=Depends(get_current_user)):
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT o.status, o.shopper_id, o.producer_id, o.stripe_payment_intent_id, o.total FROM orders o WHERE o.id = %s", (order_id,))
    order = cur.fetchone()
    if not order: cur.close(); conn.close(); raise HTTPException(404, "Order not found")
    current_status, shopper_id, producer_id, intent_id, total = order
    if user["role"] == "producer":
        cur.execute("SELECT id FROM producers WHERE user_id = %s", (user["id"],))
        prod = cur.fetchone()
    ...
    if req.status in notif_map:
        ...
        cur.execute("INSERT INTO notifications (user_id, type, title, body, order_id) VALUES (%s,%s,%s,%s,%s)", (shopper_id, req.status, title, body, order_id))
    conn.commit(); cur.close(); conn.close()
    return {"message": f"Order status updated to {req.status}", "order_id": order_id}

def run_auto_cancel():
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT id, stripe_payment_intent_id, shopper_id FROM orders WHERE status = 'pending' AND respond_by_at < NOW()")
    expired = cur.fetchall()
    for order_id, intent_id, shopper_id in expired:
        ...
        cur.execute("INSERT INTO notifications (user_id, type, title, body, order_id) VALUES (%s,'auto_cancelled','Order Auto-Cancelled','Order #' || %s || ' was not confirmed in time. Full refund issued.',%s)", (shopper_id, order_id, order_id))
    conn.commit()
